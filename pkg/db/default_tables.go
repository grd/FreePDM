...
// SQL Document properties
type PdmDocument struct {
	gorm.Model
	// DocumentID              int `gorm:"primaryKey"` // redundant since 'gorm.Model'
	DocumentNumber          int
	DocumentName            string
	...
	UserID int // Column(Integer, ForeignKey("users.user_id"))  # One to One
	// User = relationship("PdmUser", back_populates="documents")
	ItemID int // Column(Integer, ForeignKey("items.item_id"))  # Many to One
	// Item = relationship("PdmItem", back_populates="models")
}

func (self PdmDocument) String() string {
	// TODO Needs to be implemented properly
	return fmt.Sprintf("DocumentID={self.document_id!r}, document_number={self.document_number!r}, document_name={self.document_name!r}, document_description={self.document_description!r}, document_full_description={self.document_full_description!r}, document_filename={self.document_filename!r}, document_ext={self.document_ext!r})")
}
...
// KeyValue list is a list of Key Value pairs.
// Obviously you could use a map for this but a map is *not* unstructured.
...
type PdmItem struct {
	gorm.Model
	ItemVersion           int16
	ItemVersionString     string // the pretty printed ItemVersion
	ItemName              string
	ItemDescription       string
	ItemFullDescription   string
	ItemKeyValueList      []KeyValue // For example "Owner=Gerard" and "Drafter=Joe"
	ItemNumberLinkedFiles []int      // The assembly parts
	ItemPath              string
	ItemPreviewImage      []byte // TODO item from Model

	//item should be able to exist in multiple projects. but need a single store location

	// relationships with other tables
	UserId int //ForeignKey("users.user_id"))  # Many to one
	// user = relationship("PdmUser", back_populates="items")  # Many to Many
	ProductId int // Column(Integer, ForeignKey('products.product_id'), nullable=False)  // Many to Many
	// One Item can have many Models OR Documenets
	Models    []int // relationship("PdmModel", back_populates="item")  // One to Many
	Documents []int // relationship("PdmDocument", back_populates="item")  // One to Many
	// One item can have one material otherwise it is another Item...
	Materials  []int // relationship("PdmMaterial", back_populates="item", uselist=False)  // One to one
	Purchasing []int // relationship("PdmPurchasing", back_populates="item", uselist=False)  // Many to Many
}
